...
string IniParser::operator()(const string& s_group, const string& s_key)
{
	...
	{
		throw new invalid_argument("The ini file was corrupt, and data cannot be accessed.");
	} // end else
} // end operator()
...
void IniParser::parseFile(const string& s_file)
{
	...
	{
		b_fileGood = false;
		cout << "Parsing of file " << s_file << " failed, file is either corrupt or wasn't found." << endl;
		return;
	} // end else
...
bool IniParser::isFileGood(void)
{
	return b_fileGood;
} // end method isFileGood
